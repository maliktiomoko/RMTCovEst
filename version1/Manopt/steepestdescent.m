...
%       Now using storedb.remove() to keep the cache lean.
    ...
    localdefaults.minstepsize = 1e-20;
...
%         if iter==0
%             newx = problem.M.retr(x, -grad, 1/(iter+1));
%             stepsize=1/(iter+1);
%             newkey = storedb.getNewKey();
%             lsstats.costevals = 1;
%             lsstats.stepsize = stepsize;
%             lsstats.alpha = stepsize;
         [stepsize, newx, lsstats] = options.linesearch( ...
