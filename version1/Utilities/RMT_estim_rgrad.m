...
switch distance
    case 'Fisher'
        V=Cs./S;taille=size(S);p=taille(1);c2=p/n2;
        %if sum(sum(isnan(V)))==0
        %[K,V]=eig((F));
        [lambda,indi]=sort(diag(V));
        ...
        %out_prim=zeros(p,1);
        M1=bsxfun(@rdivide,(2/p)*log(lambda'),bsxfun(@minus,lambda,lambda'))';
        M2=bsxfun(@rdivide,(-2/p)*log(zeta'),bsxfun(@minus,lambda,zeta'));
        M3=bsxfun(@rdivide,-1,bsxfun(@minus,lambda,lambda'));
        M4=bsxfun(@rdivide,1,bsxfun(@minus,lambda,zeta'));
        M1(logical(eye(size(M1))))=[];M3(logical(eye(size(M3))))=[];
        M1=reshape(M1,p-1,p)';M3=-reshape(M3,p-1,p)';
        diag_out=(2/p)*log(lambda).*(sum(M4,2)+sum(M3,2)-1./lambda)+sum(M2,2)+sum(M1,2)-(2-2*log(1-c2))./(p*lambda);
        vec=diag_out(indi);
        my_out=Cs.*diag(vec);
        out_1=real((my_out+transpose(my_out))/2);
        [az,r]=RMT_estim(S,Cs,n2,'Fisher');
        out_=2*r*sqrt(az)*out_1;
%         else
%             out_=10*eye(p);
%             out_1=out_;
%         end
